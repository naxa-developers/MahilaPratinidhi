...
class HlcitViewSet(ReadOnlyModelViewSet):
    ...
    def get_queryset(self, *args, **kwargs):
        names = {}
        n = []
        rastriya_names = RastriyaShava.objects.values('english_name', 'id').filter(hlcit_code=self.kwargs['hlcit'])
        province_names = ProvinceMahilaPratinidhiForm.objects.values('english_name', 'id').filter(hlcit_code=self.kwargs['hlcit'])
        federal_names = PratinidhiShava.objects.values('english_name', 'id').filter(hlcit_code=self.kwargs['hlcit'])
        # m = rastriya_names.__class.__name
        # import ipdb
        # ipdb.set_trace()
        all_names = list(chain(rastriya_names, province_names, federal_names))
        for nama in all_names:
            names['name'] = nama['english_name']
            names['id'] = nama['id']
            names['model'] = "hello"
            n.append(dict(names))
